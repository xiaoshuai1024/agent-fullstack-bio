...
搜索 `docs/products/` 下的 PRD 文档，至少阅读以下范例了解项目 PRD 风格：

- `docs/products/v2/康豆平台v2-PRD.md` — 产品级 PRD 范本
- `docs/products/v1/多租户数据共享-PRD.md` — 跨系统 PRD 范本

### 3. 判断版本路径

根据功能归属自动选择路径：

- **v2 新功能**（运营增长、多行业 SaaS、结算分佣、独立小程序等）→ `docs/products/v2/`
...
> 例：给平台商家提供一套无需开发的次日达配送工具，让中小商家 10 分钟上线同城配送能力，抢回被外卖平台分走的订单。
...
- **发布方式**：{{全量发布 / 开关控制一键切换}}
- **回滚条件**：{{什么情况下回滚}}
- **功能开关**：{{FeatureGate 开关 key、作用域、关闭时行为}}
...
- **状态机**：`stateDiagram-v2`，`[*]` 表示初始/终止态。注意部分渲染器对 `stateDiagram-v2` 支持不佳，若无法渲染可用带箭头的 ASCII 文本图替代
...
- PRD **不涉及具体代码实现**，但可包含简单的技术选型说明（如支付渠道、数据存储策略等）
